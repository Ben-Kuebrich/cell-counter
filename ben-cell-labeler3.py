...
class simpleapp_tk(Tkinter.Tk):
	...
	def LoadVariablesFromFolder(self):
		...
		try:
			...
			autoPointFile = open('absolute.txt', 'r')
			print "Variables loaded From folder: "+os.getcwd()
			...
			print "Failed to load variables from folder: "+directory+" Check if it contains the document absolute.txt"
			print e
			variables="undefined"
		else:
			pass
...
def checkPoints(pointsX, pointsY, autoPointsX, autoPointsY, w,h,minArea,bg_with_keypoints,noGradientDescent,imageFile):
...
### I don't think need these two lines
	...
	for autoPoint in range(len(autoPointsX)):
		...
		if match==0:
			if noGradientDescent==1:
				### Draw yellow box around the false positives
	...
	for point in range(len(pointsX)):
	 	if noGradientDescent==1:
	 		### Draw green around the false negatives
...
def runMain(directory,variables,label1,label2,label3,label4, deltaDelta, roundsRounds, randomRandom, fpWeight, tpWeight, test, noCheck,conservative, troubleshoot):
	...
	while continuous==1:
		...
		while improving < roundsAfterStuck+1:
			...
			print "last round ended with cost:",bestCost,"new round's variables starting at:",variables
			print "#################################"
			print "#################################"
			...
			for var in range(len(nextVars)):
				...
				for imageFile in range(0,len(ims)):
					...
					if noCheck != 1:
						...
						TP, FP, FN, a = checkPoints(pointsX, pointsY, autoPointsX, autoPointsY, w,h,minArea,bg_with_keypoints,noGradientDescent,imageFile)
				...
				if noCheck != 1:
					...
					if first==1:
						...
						if noGradientDescent!=1:
							...
							a, b, c, bg_with_keypoints = checkPoints(pointsX, pointsY, autoPointsX, autoPointsY, w,h,minArea,bg_with_keypoints,noGradientDescent,imageFile)
					...
					for item in range(0,len(TPs)):
						print "SECTION ",item,"  Sensitivity:",TPs[item]*1.0/(TPs[item]+FNs[item]),"  Accuracy:",TPs[item]*1.0/max(1,(TPs[item]+FPs[item]))
